...
# --- 設定網頁 ---
st.set_page_config(page_title="ELN 智能戰情室 (Step-Down版)", layout="wide")
...
# 🌟 解析 KO 設定 (支援 Step Down)
def parse_ko_settings(ko_price_val):
    """
    解析 KO 價格欄位，回傳 (初始KO, 遞減幅度)
    支援格式: "100", "100(2%遞減)", "98 (Step 1%)"
    """
...
# 🌟 判斷是否為 Period End (月月比)
def is_period_end_check(ko_type_val):
    s = str(ko_type_val).upper().strip()
    if "PERIOD END" in s or "MONTHLY" in s:
        return True
    return False
...
def clean_percentage(val):
    ...
    try:
        # 這裡只處理 KI 或 Strike 的純數字百分比，KO 有專屬解析函數
        s = str(val).replace('%', '').replace(',', '').strip()
        # 如果包含括號(Step down)，只取前面的數字
        s = re.split(r'[\(（]', s)[0]
        return float(s)
    except: return None
...
# --- 主畫面 ---
st.title("📊 ELN 智能戰情室 - Step-Down 版")

uploaded_file = st.file_uploader("請上傳 Excel (支援 Step Down / Period End)", type=['xlsx', 'csv'], key="uploader")
...
if uploaded_file is not None:
    try:
        ...
        for index, row in clean_df.iterrows():
            ...
            is_period_end = is_period_end_check(row['KO_Type']) # 🌟 檢查是否為 Period End
            ...
            if row['IssueDate'] <= today_ts:
                ...
                if not backtest_data.empty:
                    for date, prices in backtest_data.iterrows():
                        ...
                        # 🌟 判斷今天是否為觀察日 (針對 Period End)
                        # 簡易判斷：日期號數相同 (需考慮大小月，暫用 relativedelta 近似)
                        is_obs_date = True
                        if is_period_end:
                            # 只有在發行日的同一號才檢查 (例如每個月20號)
                            # 如果當月沒有20號(如2月)，通常延後，這裡做簡易判斷：只檢查是否滿整月
                            # 或是更簡單：直接檢查這一天是否為「每月紀念日」附近
                        ...
                        if date.day < row['IssueDate'].day: m_pass -= 1 # 還沒滿月
                        if m_pass < 0: m_pass = 0
                        
                        # 假設第一個月(Obs 1)就開始降? 還是第二個月? 
                        # 通常: Obs 1 = 100, Obs 2 = 98... => (m_pass) * step
                        # 或者: Obs 1 = 98... => (m_pass + 1) * step
                        # 這裡採用: 滿一個月後開始降 (Month 1 Obs: 100, Month 2 Obs: 98)
                        ...
                        for asset in assets:
                            ...
                            # KO (Period End 只在特定日看)
                            if not asset['locked_ko']:
                                if is_post_nc:
                                    # 🌟 如果是 Period End，非觀察日不檢查 KO
            ...
            for i, asset in enumerate(assets):
                if asset['price'] > 0:
                    ...
                    # 🌟 Shadow KO 判斷 (現在是否超過目前的 KO 門檻)
                    if asset['perf'] >= current_ko_thresh and not asset['locked_ko']:
            ...
            if today_ts < row['IssueDate']:
                ...
                if today_ts < nc_end_date:
                    ...
                    if is_period_end:
                        status_msgs.append(f"👀 比價中 (月月比)")
                    else:
                        status_msgs.append("👀 比價中 (Daily)")
                
                # 🌟 顯示目前的 KO 門檻 (如果是 Step Down)
                if ko_step_val > 0:
                    status_msgs.append(f"📉 目前KO門檻: {current_ko_pct}%")
