...
SENDER_PASSWORD = "你的16位數應用程式密碼"

# 2. LINE 設定 (通知自己用)
LINE_ACCESS_TOKEN = "你的Channel_Access_Token"  # 長得像亂碼很長那一串
MY_LINE_USER_ID = "你的User_ID"                # U開頭的那一串
# ==========================================
...
with st.sidebar:
    ...
    if SENDER_EMAIL and SENDER_PASSWORD:
        st.success(f"✅ Email 系統就緒")
    else:
        st.error("⚠️ 請設定 Email")
        
    st.markdown("---")
    st.header("💬 LINE 通知 (給自己)")
    if LINE_ACCESS_TOKEN and MY_LINE_USER_ID:
        st.success(f"✅ LINE 系統就緒")
    else:
        st.warning("⚠️ 請填寫 LINE Token 與 UserID")

    st.markdown("---")
    st.header("🕰️ 時光機設定")
    simulated_today = st.date_input("設定「今天」日期", datetime.now())
    st.caption(f"模擬日期：{simulated_today.strftime('%Y-%m-%d')}")
    st.info("💡 **功能更新：**\n新增 LINE 智能摘要通知 (僅彙整重要事項傳送給管理者)")

# --- 函數：發送 Email ---
def send_email(receiver, subject, body):
    if not SENDER_EMAIL or not SENDER_PASSWORD or not receiver:
        st.warning("⚠️ Email 資料不完整")
        return False
    msg = MIMEMultipart()
    ...
    try:
        ...
        server.quit()
        st.toast(f"✅ Email 已寄給 {receiver}", icon="📩")
        return True
    ...
    try:
        ...
        for i in range(1, 6):
            ...
            if tx_idx < len(df.columns):
                ...
                clean_df[f'T{i}_Code'] = ""; clean_df[f'T{i}_Strike'] = 0
        ...
        # 5. 運算邏輯
        results = []
        line_alert_list = [] # 用來存 LINE 的摘要內容
        ...
        for index, row in clean_df.iterrows():
            ...
            row_res = {
                "債券代號": row['ID'], "天期": row['Tenure'], "收件人": row['Name'], "Email": str(row['Email']).strip(),
            ...
            for idx, row in final_df.iterrows():
                if any(x in row['狀態'] for x in ["提前", "到期", "已破", "獲利", "接股"]):
                    ...
                    if st.button(f"📧 通知 {row['債券代號']}", key=f"btn_{idx}"):
                        if sender_email: send_email(email, row['msg_subject'], row['msg_body'])
                        else: st.error("Email 設定不完整")
